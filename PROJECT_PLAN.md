...
**Фаза:** Phase 7 (багфиксы + аудит + новые фичи) — в процессе
...
**Прогресс Phase 7:** см. раздел Phase 7 ниже
...
## Phase 7 — Багфиксы, аудит и новые фичи ✅ DONE (сессия 2026-03-19)

### Багфиксы
- [x] **Правый клик на карточке** — контекстное меню через `createPortal`, устранён конфликт с drag-захватом (`releasePointerCapture`)
- [x] **Чеклист toggle** — исправлен для react-markdown v10, позиция через `li` node.position
- [x] **Drop подсветка колонки** — `overColumnId` tracking + inline outline, подсвечивается вся колонка
- [x] **Перенос задачи на доску** — авто-создание колонок при переносе, fly-away анимация
- [x] **Модалки не закрываются по клику на backdrop** — исправлено поведение backdrop click

### Аудит кода
- [x] **Security: path traversal** — защита от `../` в именах файлов
- [x] **Security: SSRF Ollama** — валидация URL, только localhost/LAN адреса
- [x] **Data loss: importAllData** — добавлены недостающие поля при импорте
- [x] **Memory leak: FocusWindow** — устранён двойной interval
- [x] **Stale closures** — App.tsx и AIAssistantDialog (замыкания на устаревшие данные)
- [x] **Дедупликация** — утилиты чеклистов вынесены в общий модуль
- [x] **Мёртвый код** — удалены неиспользуемые IPC константы, мёртвая ветка `Array.isArray`

### Новые фичи
- [x] **Task Doctor** — визард аудита здоровья задач (8 диагнозов + quick-fix кнопки)
- [x] **Projects Canvas** — канвас проектов с карточками (Confluence, ПАП, ПМИ, РП, архитектор)
- [x] **Типы колонок** — поле `column_type` (backlog/in_progress/waiting/done/cancelled), выбор в ColumnEditor
- [x] **Кнопка ✓ на карточке** — быстрое завершение задачи (перенос в колонку done) одним кликом
- [x] **Управление уведомлениями** — режим "Не беспокоить" + настройка интервала автоматизации
- [x] **Ollama LAN** — разрешены не только localhost URL, поддержка локальной сети

---

## Phase 8 — Идеи (backlog)
