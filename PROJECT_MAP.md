...
**Task Grabber** — Кроссплатформенное (Windows/macOS/Linux) tray-приложение для мгновенного создания задач из любого места: выделенный текст, файлы, письма Outlook (.msg). Включает канбан-доску с несколькими досками, Focus Mode (Pomodoro), Desktop Widget, AI-ассистент, Smart Rules, глобальный поиск, Timeline/Calendar view.
...
### Task/TaskCard.tsx
Карточка задачи: приоритет (цветная полоса), теги, статус-бейдж (column_type), последний апдейт или дата создания, счётчик апдейтов, вложения. ПКМ: быстрый апдейт + перенос на доску. Поддержка batch selection.
...
### Files/BoardFilesView.tsx
Файловый менеджер доски: загрузка файлов (drag&drop + диалог), поиск, контекстное меню (открыть, привязать к задаче, удалить). Секция «Файлы из задач» — все аттачменты задач текущей доски (read-only, с названием задачи).
...
| `tasks:move` | Переместить + автозаполнение completed_at |
| `tags:update` | Обновить тег (цвет, имя) |
| `taskUpdates:get` | Апдейты задачи |
| `taskUpdates:create` | Создать апдейт |
| `taskUpdates:update` | Обновить апдейт (текст, дату) |
| `taskUpdates:delete` | Удалить апдейт |
| `taskUpdates:counts` | Количество апдейтов по task_id[] |
| `taskUpdates:latest` | Последний апдейт по task_id[] |
| `boardFiles:taskAttachments` | Аттачменты задач доски |
...
CREATE TABLE tasks (
  ...
  completed_at    TEXT,                  -- Дата выполнения (автозаполнение при move в done)
  created_at TEXT DEFAULT (datetime('now')),
  updated_at TEXT DEFAULT (datetime('now'))
);
```
...
### Сборка
Vite 5 (renderer) + tsc (main) + electron-builder 26. CI/CD через GitHub Actions: автобилд по тегу для Windows (.exe), macOS (.dmg x64+arm64), Linux (.AppImage + .rpm).
